...
export class AppComponent implements OnInit{
  ...
  @ViewChild('remoteVideo',{static: true}) remoteVideoHtmlElement : ElementRef;
  iceConfiguration = {
    iceServers :[
    ...
    this.socket.on('send-message',(body) => {
      ...
      if(!this.myOwnMessage) {
        ...
        if(body?.info === '1') {
          ...
          this.localPeerConnection.setRemoteDescription(new RTCSessionDescription(body.message))
            .then(()=> {
              this.localPeerConnection.createAnswer().then (answer => {
                this.localPeerConnection.setLocalDescription(answer).then(() => {
  ...
  initiateWebRtc() {
    ...
    navigator.mediaDevices.getUserMedia({audio: true,video : true}).then(
      localMediaStream => {
        ...
        this.remoteVideoHtmlElement.nativeElement.srcObject = this.remoteMediaStream
